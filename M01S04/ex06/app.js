...
console.log(sumYears + ''); //this makes the type conversion to string, also you can use .toString().
...
for (var i = 0; i < person.friends.length; i++) {
  ...
  totalYears = totalYears + friend.age;
}
...
for (var i = 0; i < person.friends.length; i++) {
  var friend = person.friends[i];
  var ageDiff = Math.abs(person.age - friend.age); //this gets the absolute value of the difference, so no negative numbers.
